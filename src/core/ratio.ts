import { is_int, is_prime, simplify_ratio, factorize, factorize_int } from './math';
...
export class Ratio {
    ...
    update( payload?: Ratio | factorisation | number | number[]){ 
        ...
        if (obj_not_array(payload)){
            ...
            Object.keys(fact).forEach(p => {
                if (!is_int(fact[p]) || !is_prime(parseInt(p))) throw new Error("corrupted factorisation");
    ...
    static mul(a:any, b:any){
        ...
        const primes = new Set<string>([...Object.keys(a.fact), ...Object.keys(b.fact)]);
        console.log(primes);
